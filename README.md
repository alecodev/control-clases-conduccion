# Control de clases de conducción A2 + B1

Aplicación web para llevar el control de las clases teóricas necesarias durante el proceso de formación para las licencias de conducción **A2** y **B1**.


## ¿Qué permite hacer?

- Consultar las 17 clases organizadas por área temática.
- Identificar cuáles clases corresponden a A2, B1 o a ambas categorías.
- Marcar las clases vistas y conocer cuántas quedan pendientes.
- Filtrar el listado por tipo de licencia y estado de avance.
- Limpiar el progreso para comenzar un nuevo seguimiento.
- Copiar una URL que conserva el estado actual.

## Contenido de las clases

El seguimiento incluye las siguientes áreas:

- Normas de Tránsito.
- Movilidad Segura y Sostenible.
- Señalización e Infraestructura Vial.
- El Vehículo / Taller.
- Teoría Específica para A2 y B1.

## Privacidad y almacenamiento

La aplicación no requiere registro y no envía información a un servidor. El avance se guarda dentro de la propia URL, por lo que puede conservarse o compartirse copiando el enlace completo.

## Licencia

Este proyecto se distribuye bajo la [GNU General Public License v3.0](LICENSE).
